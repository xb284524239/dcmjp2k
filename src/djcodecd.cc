...
OFBool DcmJp2kDecoderBase::canChangeCoding(const E_TransferSyntax oldRepType, const E_TransferSyntax newRepType) const {
    ...
    if (newRep.isNotEncapsulated() &&
        ((oldRepType == EXS_JPEG2000LosslessOnly) || (oldRepType == EXS_JPEG2000) || (oldRepType == EXS_JPEG2000MulticomponentLosslessOnly) || (oldRepType == EXS_JPEG2000Multicomponent)))
        return OFTrue;
...
OFCondition DcmJp2kDecoderBase::decode(const DcmRepresentationParameter * /* fromRepParam */, DcmPixelSequence *pixSeq, DcmPolymorphOBOW &uncompressedPixelData, const DcmCodecParameter *cp,
    ...
    if (imageFrames >= OFstatic_cast(Sint32, pixSeq->card())) imageFrames = pixSeq->card() - 1;  // limit number of frames to number of pixel items - 1
    if (imageFrames < 1) imageFrames = 1;                                                        // default in case the number of frames attribute is absent or contains garbage
...
OFCondition DcmJp2kDecoderBase::decodeFrame(const DcmRepresentationParameter * /* fromParam */, DcmPixelSequence *fromPixSeq, const DcmCodecParameter *cp, DcmItem *dataset, Uint32 frameNo,
    ...
    if (imageFrames >= OFstatic_cast(Sint32, fromPixSeq->card())) imageFrames = fromPixSeq->card() - 1;  // limit number of frames to number of pixel items - 1
    if (imageFrames < 1) imageFrames = 1;                                                                // default in case the number of frames attribute is absent or contains garbage
...
OFCondition copyUint32ToUint8(opj_image_t *image, Uint8 *imageFrame, Uint16 columns, Uint16 rows);

OFCondition copyUint32ToUint16(opj_image_t *image, Uint16 *imageFrame, Uint16 columns, Uint16 rows);

OFCondition copyRGBUint8ToRGBUint8(opj_image_t *image, Uint8 *imageFrame, Uint16 columns, Uint16 rows);
...
OFCondition DcmJp2kDecoderBase::decodeFrame(DcmPixelSequence *fromPixSeq, const DcmJp2kCodecParameter *cp, DcmItem *dataset, Uint32 frameNo, Uint32 &currentItem, void *buffer, Uint32 bufSize,
    ...
    if (imageSamplesPerPixel > 1) {
        switch (cp->getPlanarConfiguration()) {
            case EJ2KPC_restore:
                ...
                if (imagePlanarConfiguration > 1) imagePlanarConfiguration = determinePlanarConfiguration(imageSopClass, imagePhotometricInterpretation);
                break;
            case EJ2KPC_auto:
    ...
    if (result.good()) {
        ...
        if (jlsData[compressedSize - 1] == 0) compressedSize--;
...
#define J2K_CODESTREAM_MAGIC "\xff\x4f\xff\x51"
        ...
        if (result.good()) {
            if (image->x1 != imageColumns)
                result = EC_J2KImageDataMismatch;
            else if (image->y1 != imageRows)
                result = EC_J2KImageDataMismatch;
            else if (image->numcomps != imageSamplesPerPixel)
                result = EC_J2KImageDataMismatch;
        ...
        if (!result.good()) {
            ...
            if (result.good()) {
                ...
                {
                    if (image->comps[0].prec <= 8) copyUint32ToUint8(image, OFreinterpret_cast(Uint8 *, buffer), imageColumns, imageRows);
                    if (image->comps[0].prec > 8) copyUint32ToUint16(image, OFreinterpret_cast(Uint16 *, buffer), imageColumns, imageRows);
...
Uint16 DcmJp2kDecoderBase::determinePlanarConfiguration(const OFString &sopClassUID, const OFString &photometricInterpretation) {
    ...
    if (photometricInterpretation == "YBR_FULL") {
        if ((sopClassUID == UID_UltrasoundMultiframeImageStorage) || (sopClassUID == UID_UltrasoundImageStorage)) return 1;
    }
